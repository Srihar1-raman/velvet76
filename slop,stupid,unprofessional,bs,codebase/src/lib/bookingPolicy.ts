...
const SERVICE_AREA_ERROR = "Currently serving Delhi, Gurugram, and IGI Airport only.";
...
function flattenAddress(place: PolicyPlace): string {
  const parts = [
    ...
    place.name ?? "",
    ...(place.address_components ?? []).flatMap((c) => [c.long_name ?? "", c.short_name ?? ""]),
  ];
...
export function validateServiceArea(place: PolicyPlace): { ok: boolean; message: string } {
  if (!place.place_id) {
...
export function minimumLeadHours(now: Date, isAirportGurugramSubcase: boolean): number {
  if (isAirportGurugramSubcase && now.getHours() >= LATE_NIGHT_HOUR) {
...
export function meetsLeadTimePolicy(
  ...
  if (Number.isNaN(selected.getTime())) return { ok: false, minHours: BASE_MIN_HOURS };
...
export function isAirportGurugramSubcase(pickup: string, drop: string): boolean {
